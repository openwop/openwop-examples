...
> **Suite-version note (drift #8, 2026-08-25).** Every bundle row below was measured against `@openwop/openwop-conformance@1.130.0`. The suite is now **1.139.0**, and three of the changes between them alter what a run *reports* rather than what a host *does*, so the counts below are **not comparable** to a 1.139.0 run: a run-end disposition summary now surfaces `blocked` / `skipped` / `inapplicable` rows that previously printed as passes; `forkDeclined` treats a `404` on the fork route as `blocked` where it used to hard-fail; and `a2a-task-roundtrip`'s push-egress leg became two-sided. **No re-measurement has been done** — this is a marker, not a new claim, and the honest reading of the rows below is "what 1.130.0 saw on this host on the date given".


> **Delivery-header correction — 2026-08-25.** This host emitted `X-openwop-Subscription-Id`, `X-openwop-Signature-Timestamp`, a **bare-hex** `X-openwop-Signature`, no `X-openwop-Event-Type`, and a runtime-default `User-Agent`. `webhooks.md` §"Delivery headers" specifies `X-openwop-Webhook-Id`, `X-openwop-Timestamp`, `X-openwop-Signature: sha256={hex}`, `X-openwop-Event-Type`, and `openwop-webhook-dispatcher/{version}`. The signed **bytes** were always correct (`{timestamp}.{rawBody}`); only the envelope was wrong. All three reference hosts had the same defect — the Python host is a port of the SQLite one, and the suite had asserted the same invented names until it was corrected against the spec in [#914](https://github.com/openwop/openwop/pull/914) (2026-08-09) after a tier-2 host emitted what the spec shows and failed.
>
> **Why the measurements below never caught it — four different mechanisms, one green.** (1) Runs on suites older than 2026-08-09 asserted the same invented names this host emitted: suite and host were mirrors of each other, and the spec was the oracle neither was checked against. (2) `--certify` runs on a **default-env boot** record `webhook-signed-delivery` as `blocked` — the SSRF guard refuses the loopback receiver — so the assertions were never reached. (3) The one run anywhere in the reference set that *did* execute these assertions with the opt-in on — **this host**, 2026-08-16, suite 1.116.0 — went `executed-fail` and was recorded as **"not diagnosed here"**. A defect can survive being measured. (4) The corpus's own `postgres-host-conformance` job **runs this scenario with the opt-in on, on every push to `main`, and it has failed at `webhook-signed-delivery.test.ts:194` every time since 2026-08-09** — absorbed by an **85% pass-rate floor** the run clears with room to spare — the 2026-08-20 `main` run measured `passed=2907 total=3031 pass-rate=95%` and exited 0 with that assertion red. **A pass-rate floor converts a specific, named, reproducible assertion failure into an anonymous fraction**, and a ten-point cushion is wide enough that no single scenario can move the verdict. This is the mechanism with a human-facing green checkmark on it. *(Corrected 2026-08-25: this sentence first cited `588/682 = 86%`, numbers that are a **comment** in `postgres-host-conformance.yml:166` echoed by Actions, not output from any run. The failure was real; the arithmetic was read out of prose that looked like a measurement. **A log is not a measurement — it is a transcript containing measurements, comments, and echoed source, typographically identical.**)* **Confirmed after the fix:** the 2026-08-25 `main` run shows this scenario green at 772 ms.
>
> Re-measured 2026-08-25 after the fix: `webhook-signed-delivery` **executed-pass** (real witness, ~0.5 s — not the sub-50 ms shape a soft-skip produces), alongside `webhook-negative` and `webhook-tenant-isolation`. Red-before-green confirmed by sabotage: reverting `X-openwop-Webhook-Id` to `X-openwop-Subscription-Id` reddens the assertion. A full `--certify` re-run has **not** been done, so every bundle row below still describes the pre-fix host.


> **Latest measurement — bundle v2 (RFC 0148 §C), 2026-08-16, `@openwop/openwop-conformance@1.130.0` (`--certify --bundle-version 2`, requirement ledger on):** `openwop-host-postgres@1.0.0` — executed-pass 230 / executed-fail 2 / skipped 0 / inapplicable 138 / blocked 96 (**0** zero-assertion passes). Per-profile: certifiable: `openwop-discovery-core` (+ alias `openwop-core`), `openwop-stream-sse`, `openwop-stream-poll`, `openwop-provider-policy`, `openwop-memory` (both now have floors — suite 1.120.0 closed RFC 0148 G7 — and both certify here), `openwop-fixtures`; `openwop-node-packs` **not held** (runtime-derived). The 2 `executed-fail` rows are the same two as the 1.116.0 run: `webhook-signed-delivery` (delivery omits `X-openwop-Webhook-Id`, `webhooks.md` §"Delivery headers" — a host gap) and `route-coverage` (passes when re-run alone; timing under the full run — not diagnosed here). The bundle is committed as [`certification-bundle-v2.json`](./certification-bundle-v2.json); verify with `verifyBundleV2` from the suite. Local boot, default env, single run.
>
> **Prior measurement (superseded 2026-08-16 by the 1.130.0 run) — bundle v2 (RFC 0148 §C), 2026-08-16, `@openwop/openwop-conformance@1.116.0` (`--certify --bundle-version 2`, requirement ledger on):** `openwop-host-postgres@1.0.0` — executed-pass 353 / executed-fail 2 / skipped 0 / inapplicable 74 / blocked 32 (125 of the passes carry `assertionCount: 0`). Per-profile: certifiable: `openwop-discovery-core` (+ alias `openwop-core`), `openwop-stream-sse`, `openwop-stream-poll`, `openwop-fixtures`; **not** certifiable: `openwop-provider-policy` and `openwop-memory` (no floor defined — unprovable, RFC 0148 §C G6), `openwop-node-packs` (REJECTED — `openwop.floor.pack-registry` and `openwop.floor.pack-registry-publish` executed-pass with zero assertions). The 2 `executed-fail` rows are `stream-subscribe-from-beginning.test.ts` and `webhook-signed-delivery.test.ts` (pglite-backed local boot, `OPENWOP_WEBHOOK_ALLOW_PRIVATE=true`; not diagnosed here). The bundle is committed as [`certification-bundle-v2.json`](./certification-bundle-v2.json); it validates against `certification-bundle-v2.schema.json` and the consumer verifier `verifyBundleV2` agrees with the emitter row-for-row. Under RFC 0148 §A a bundle with `blocked > 0` does not certify the claims it blocks; the per-profile verdicts above are the state of the evidence. The pre-ledger measurements below counted an early-returning test as a pass and could not tell `skipped` from `inapplicable` from `blocked`; they are kept as the dated record and superseded by this line.

> **Latest measurement (2026-06-11 post-fix re-run, suite `@openwop/openwop-conformance@1.22.0`):** **1848 passed / 0 failed / 115 skipped / 0 todo of 1963 tests (94.1% total) — 0 deterministic failures**, re-measured via pglite + `OPENWOP_WEBHOOK_ALLOW_PRIVATE=true` (full 82-fixture catalog resolved via the new sibling-checkout fixture probe — no symlinks). The 2 RFC 0093 §A3 `webhook-tenant-isolation` failures from the same-day reading below are **closed**: the host now enforces the registration-time tenant-membership gate (403 `tenant_membership_required` on a foreign `tenantId`, tenant-scoped `DELETE` with no existence leak, tenant-filtered delivery fan-out) — `webhook-tenant-isolation.test.ts` passes 4/4 non-vacuously. Restores the 0-deterministic-failure posture.
>
> **Prior measurement (superseded same-day by the webhook-tenant-gate fix; 2026-06-11, suite `@openwop/openwop-conformance@1.22.0`):** **1846 passed / 2 failed / 115 skipped / 0 todo of 1963 tests (94.0% total)**, re-measured via pglite + `OPENWOP_WEBHOOK_ALLOW_PRIVATE=true` (cross-host re-measurement in `INTEROP-MATRIX.md`). **The 2 failures are real**: the new RFC 0093 §A3 `webhook-tenant-isolation.test.ts` — this host advertises `webhooks.supported: true` but lacks a registration-time tenant-membership gate (genuine gap on an advertised surface; fix tracked). First reading to break the 0-deterministic-failure posture, and honestly so. **Measurement-basis note:** this run loads the full 82-fixture catalog (prior readings ran with degraded fixture loading — the host's upward fixture probe only finds the catalog when nested inside the spec repo), so passed/skipped deltas vs 1.21.0 reflect broader coverage.
>
> **Prior measurement (superseded 2026-06-11; 2026-06-09, suite `@openwop/openwop-conformance@1.21.0`):** **1746 passed / 0 failed / 208 skipped / 0 todo of 1954 tests (89.4% total) — 0 deterministic failures**, re-measured via pglite + `OPENWOP_WEBHOOK_ALLOW_PRIVATE=true`. Holds the 0-failure production-profile posture against 1.21.0; the new RFC 0090/0091/0092 probes soft-skip (profiles not advertised). The cleanest reference host.
>
> **Prior measurement (superseded 2026-06-09; 2026-06-01, suite `@openwop/openwop-conformance@1.15.0`):** **2050 passed / 0 failed / 93 skipped / 0 todo of 2143 tests (95.7% total) — 0 deterministic failures**, re-measured via pglite + `OPENWOP_WEBHOOK_ALLOW_PRIVATE=true`. Cleaner than the prior 2026-05-30 reading (1994/8): the 8 full-suite-parallelism flakes (`aiEnvelope.schemaDrift` ×4, `model-capability-substituted` ×3, `highConcurrency` ×1) did not recur in this run. Total rose 2094 → 2143 as the v1.11–v1.15 scenarios landed. The TypeScript SDK `dist/` was built first so the server-free corpus-validity scenarios pass. The cleanest reference host. See `INTEROP-MATRIX.md` §"External conformance suite — pass rates".
>
> **Prior measurement (superseded 2026-06-01; 2026-05-30, suite `@openwop/openwop-conformance@1.10.0`):** **1994 passed / 8 failed / 92 skipped / 0 todo of 2094 tests (95.2% total) — 0 deterministic failures**, re-measured after the agent-memory/pack implementation (#378), the root-`limits` fix (#376), and the RFC 0006 CP-1 low-confidence suspend (#384). The agent-memory (×5), agent-packs (×3), `aiEnvelope.capBreached` (×3), `discovery` (×1) and `orchestratorConservativePath` (CP-1) failures are all closed. The 8 full-suite residuals (`aiEnvelope.schemaDrift` ×4, `model-capability-substituted` ×3, `highConcurrency` ×1) each pass in isolation on a fresh pglite — full-suite-parallelism flakes. None block a v1 production-profile MUST. See `docs/CONFORMANCE-RUNS-2026-05-30.md` + `INTEROP-MATRIX.md`.
>
> **Prior measurement (superseded by #384):** **1988 passed / 6 failed / 92 skipped of 2086 tests (95.3%) — 1 deterministic failure** (`orchestratorConservativePath`, the CP-1 suspend, since closed).
>
> **Prior measurement (superseded by #376 + #378):** **2026-05-30 — 1968 passed / 14 failed / 92 skipped of 2074 tests (94.9%)**. The 14 included the agent-memory (×5) + agent-pack (×3) + `aiEnvelope.capBreached` (×3) + `discovery` (×1) failures since closed, plus the still-open `orchestratorConservativePath` (CP-1) + timing flakes.
>
