...
> **Latest measurement (2026-05-25, post main-merge):** **1674 passed / 1 failed / 118 skipped of 1793 tests** against the current `conformance/` suite running with pglite + `OPENWOP_WEBHOOK_ALLOW_PRIVATE=true`. See `docs/CONFORMANCE-RUNS-2026-05-23.md` §"Update 2026-05-25" for the per-failure taxonomy. The RFC 0022 / `artifact-auth` / model-capability failures this branch targeted are all closed (artifact-auth via an auth-before-existence route; model-capability via the RFC 0031 gate from #189; RFC 0022 ×4 via `core.identity`). The lone remaining failure is `run-execution-bounds-shape.test.ts` — a **new RFC 0058 run-duration-breach scenario** that arrived with the suite growth on `main` (1745 → 1793 tests); this host does not yet enforce `configurable.runTimeoutMs`. It fails identically on the SQLite host and is a separate RFC 0058 track, not addressed here. `webhook-signed-delivery.test.ts` (documented timing flake) passed this run. No wire-shape change; nothing blocks any v1 production-profile MUST.
>
