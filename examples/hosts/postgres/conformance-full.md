...
> **Latest measurement (2026-05-25, current suite):** **1625 passed / 2 failed / 118 skipped of 1745 tests (93.1% total; 99.88% of non-skipped)** against the current `conformance/` suite running with pglite + `OPENWOP_WEBHOOK_ALLOW_PRIVATE=true`. See `docs/CONFORMANCE-RUNS-2026-05-23.md` §"Update 2026-05-25" for the corrected per-failure taxonomy. The 2 remaining failures are both pre-existing and unrelated to the close-out tracks: `webhook-signed-delivery.test.ts` (the documented full-suite timing flake — passes in isolation) and `artifact-auth.test.ts` (the unauthenticated artifact `GET` returns 404 instead of 401 — an auth-before-existence ordering gap on the artifact endpoint). Neither is a wire-shape change; neither blocks any v1 production-profile MUST.
>
