...
> **Latest measurement (2026-05-30, suite `@openwop/openwop-conformance@1.10.0`):** **1988 passed / 6 failed / 92 skipped / 0 todo of 2086 tests (95.3% total) — 1 deterministic failure**, re-measured after the agent-memory/pack implementation (#378) + the root-`limits` fix (#376). The agent-memory (×5), agent-packs (×3), `aiEnvelope.capBreached` (×3) and `discovery` (×1) failures are all closed. The lone deterministic residual is **`orchestratorConservativePath`** (RFC 0006 CP-1 — the low-confidence supervisor suspend: the host completes the run instead of suspending it to `waiting-approval`; not yet implemented). The other 5 (`queue-ack-nack-dlq` ×2, `replay-llm-cache-key` ×3) pass in isolation — full-suite-parallelism flakes. None block a v1 production-profile MUST. See `docs/CONFORMANCE-RUNS-2026-05-30.md` + `INTEROP-MATRIX.md`.
>
