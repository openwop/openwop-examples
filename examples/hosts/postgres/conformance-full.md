...
> **Latest measurement (2026-05-25, current suite):** **1627 passed / 0 failed / 118 skipped of 1745 tests (93.2% total; 100% of non-skipped)** against the current `conformance/` suite running with pglite + `OPENWOP_WEBHOOK_ALLOW_PRIVATE=true`. See `docs/CONFORMANCE-RUNS-2026-05-23.md` §"Update 2026-05-25" for the corrected per-failure taxonomy. No deterministic failures remain; `webhook-signed-delivery.test.ts` is a documented full-suite timing flake (passes in isolation; passed in this run). The earlier `artifact-auth.test.ts` failure (unauthenticated artifact `GET` returned 404 instead of 401) was closed by adding an auth-before-existence artifact route. No wire-shape change; nothing blocks any v1 production-profile MUST.
>
