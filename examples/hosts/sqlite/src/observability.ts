...
export function startMetricLoop(db: Database.Database): void {
  if (!enabled || metricTimer) return;
  metricTimer = setInterval(() => {
    const backlog = (
      db.prepare(
        "SELECT COUNT(*) AS n FROM runs WHERE status IN ('pending', 'running', 'waiting-approval', 'waiting-input', 'waiting-external', 'paused')",
      ).get() as { n: number }
    ).n;
