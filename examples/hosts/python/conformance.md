...
> **Latest measurement — bundle v2 (RFC 0148 §C), 2026-08-16, `@openwop/openwop-conformance@1.130.0` (`--certify --bundle-version 2`, requirement ledger on):** `openwop-host-in-memory-python@1.0.0` — executed-pass 181 / executed-fail 0 / skipped 0 / inapplicable 149 / blocked 136 (**0** zero-assertion passes). Per-profile: certifiable: `openwop-discovery-core` (+ alias `openwop-core`), `openwop-stream-sse`, `openwop-stream-poll`, `openwop-fixtures`; `openwop-node-packs` **not held** (runtime-derived). Exit 0 — every claimed profile certifiable. The bundle is committed as [`certification-bundle-v2.json`](./certification-bundle-v2.json); verify with `verifyBundleV2` from the suite. Local boot, default env, single run.
>
> **Prior measurement (superseded 2026-08-16 by the 1.130.0 run) — bundle v2 (RFC 0148 §C), 2026-08-16, `@openwop/openwop-conformance@1.116.0` (`--certify --bundle-version 2`, requirement ledger on):** `openwop-host-in-memory-python@1.0.0` — executed-pass 321 / executed-fail 0 / skipped 0 / inapplicable 78 / blocked 62 (143 of the passes carry `assertionCount: 0`). Per-profile: certifiable: `openwop-discovery-core` (+ alias `openwop-core`), `openwop-stream-sse`, `openwop-stream-poll`, `openwop-fixtures`; **not** certifiable: `openwop-node-packs` (REJECTED — `openwop.floor.pack-registry` and `openwop.floor.pack-registry-publish` executed-pass with zero assertions). Zero assertion failures. The bundle is committed as [`certification-bundle-v2.json`](./certification-bundle-v2.json); it validates against `certification-bundle-v2.schema.json` and the consumer verifier `verifyBundleV2` agrees with the emitter row-for-row. Under RFC 0148 §A a bundle with `blocked > 0` does not certify the claims it blocks; the per-profile verdicts above are the state of the evidence. The pre-ledger measurements below counted an early-returning test as a pass and could not tell `skipped` from `inapplicable` from `blocked`; they are kept as the dated record and superseded by this line.

> **Latest measurement:** **2026-06-11 (post-fix re-run) against `@openwop/openwop-conformance@1.22.0` — 1786 passed / 0 failed / 177 skipped / 0 todo of 1963 tests (91.0%) — 0 deterministic failures** (default mode, host booted with `OPENWOP_WEBHOOK_ALLOW_PRIVATE=true`; full 82-fixture catalog resolved via the new sibling-checkout fixture probe — no symlinks). All 3 failures from the same-day measurement below are **closed**: (1–2) the RFC 0093 §A3 registration-time tenant-membership gate is implemented (403 `tenant_membership_required` on a foreign `tenantId`, tenant-scoped `DELETE` with no existence leak, tenant-filtered fan-out, `webhookId` in the register response) — `webhook-tenant-isolation.test.ts` passes 4/4 non-vacuously; (3) the RFC 0058 `runTimeoutMs` gap is closed — the executor arms a per-run wall-clock deadline (`min(runTimeoutMs, OPENWOP_MAX_RUN_DURATION_MS)`, default 1h, advertised as `limits.maxRunDurationMs`), emits `cap.breached {kind: 'run-duration', observed > limit}` and fails the run with `error.code = 'run_timeout'` — `run-execution-bounds-shape.test.ts` is green non-vacuously. Drive-by hardening: terminal status flips are now atomic with the terminal event append (closes a pre-existing SSE race where a `streamMode=values` subscriber could observe a terminal status before the terminal event existed and close with zero events).
>
> **Prior measurement (superseded same-day by the tenant-gate + run-timeout fixes):** **2026-06-11 against `@openwop/openwop-conformance@1.22.0` — 1783 passed / 3 failed / 177 skipped / 0 todo of 1963 tests (90.8%)** (default mode, Python 3.11; cross-host re-measurement in `INTEROP-MATRIX.md`). The 3 failures: **2 real** — the new RFC 0093 §A3 `webhook-tenant-isolation.test.ts` (this host advertises `webhooks.supported: true` without a registration-time tenant-membership gate; fix tracked) — plus the pre-existing `run-execution-bounds-shape` RFC 0058 `runTimeoutMs` gap (documented since the 1.18.1 measurement; invisible at 1.21.0 due to the degraded-fixture basis below). **Measurement-basis note:** this run loads the full 82-fixture catalog (prior readings ran with degraded fixture loading), so passed/skipped deltas vs 1.21.0 reflect broader coverage.
>
> **Prior measurement (superseded 2026-06-11):** **2026-06-09 against `@openwop/openwop-conformance@1.21.0` (run on Python 3.11 stdlib) — 1742 passed / 0 failed / 212 skipped / 0 todo of 1954 tests (89.1% total; 100% of applicable when scoped to the host's claimed `openwop-core` + `openwop-stream-poll` + `openwop-stream-sse` profile set)** (cross-host re-measurement in `INTEROP-MATRIX.md`). The prior `artifact-auth` 404-vs-401 failure is **closed** (401 now precedes any existence check), bringing deterministic host failures to **0**; the 212 skips reflect the smaller stdlib-only advertised surface.
>
