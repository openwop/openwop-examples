...
> **Latest measurement:** **2026-06-01 against `@openwop/openwop-conformance@1.15.0` (run on Python 3.9 stdlib) — 1990 passed / 2 failed / 151 skipped / 0 todo of 2143 tests (92.9% total; 100% of applicable when scoped to the host's claimed `openwop-core` + `openwop-stream-poll` + `openwop-stream-sse` profile set)**. The same 2 failures (`artifact-auth` → 404-not-401, `run-execution-bounds-shape` RFC 0058 run-timeout) are honest non-implementations outside the claimed set; the 151 skips reflect the smaller stdlib-only advertised surface (the suite grew 2074 → 2143 since the prior reading). See `INTEROP-MATRIX.md` §"External conformance suite — pass rates".
>
