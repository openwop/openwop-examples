...
> **Latest measurement — bundle v2 (RFC 0148 §C), 2026-08-16, `@openwop/openwop-conformance@1.130.0` (`--certify --bundle-version 2`, requirement ledger on):** `openwop-host-in-memory@1.1.7` — executed-pass 206 / executed-fail 30 / skipped 0 / inapplicable 119 / blocked 111 (**0** passes carry `assertionCount: 0` — the 107 zero-assertion "passes" of the 1.116.0 run are now `inapplicable` / `blocked` rows that say why, per suite 1.119.0's `softSkip` conversion and 1.125.0's note-before-`ctx.skip()` fix). Per-profile: certifiable: `openwop-discovery-core` (+ alias `openwop-core`), `openwop-fixtures`; **not** certifiable: `openwop-stream-sse`, `openwop-stream-poll` (floor scenarios in `executed-fail`, consistent with the 2026-08-13 claim withdrawal); `openwop-node-packs` is now **not held** — runtime-derived since suite 1.117.0, dropped from `claimedProfiles` because its floor rows are not witnessed passes (previously reported as REJECTED). The 30 `executed-fail` rows are the same family as before (stream-modes ×3, interrupt-* ×6, dispatch/subworkflow, pack-registry, …) — a single local run with default env; not diagnosed here. The bundle is committed as [`certification-bundle-v2.json`](./certification-bundle-v2.json); verify with `verifyBundleV2` from the suite. Local boot, default env, single run.
>
> **Prior measurement (superseded 2026-08-16 by the 1.130.0 run) — bundle v2 (RFC 0148 §C), 2026-08-16, `@openwop/openwop-conformance@1.116.0` (`--certify --bundle-version 2`, requirement ledger on):** `openwop-host-in-memory@1.1.7` — executed-pass 311 / executed-fail 30 / skipped 0 / inapplicable 77 / blocked 43 (107 of the passes carry `assertionCount: 0`). Per-profile: certifiable: `openwop-discovery-core` (+ alias `openwop-core`), `openwop-fixtures`; **not** certifiable: `openwop-stream-sse`, `openwop-stream-poll` (floor scenarios in `executed-fail`, consistent with the 2026-08-13 claim withdrawal), `openwop-node-packs` (REJECTED — `openwop.floor.pack-registry-publish` executed-pass with zero assertions). The 30 `executed-fail` rows are the same family the 2026-06-11 run showed (stream-modes ×3, interrupt-* ×6, dispatch/subworkflow, pack-registry, …) — a single local run with default env; not diagnosed here. The bundle is committed as [`certification-bundle-v2.json`](./certification-bundle-v2.json); it validates against `certification-bundle-v2.schema.json` and the consumer verifier `verifyBundleV2` agrees with the emitter row-for-row. Under RFC 0148 §A a bundle with `blocked > 0` does not certify the claims it blocks; the per-profile verdicts above are the state of the evidence. The pre-ledger measurements below counted an early-returning test as a pass and could not tell `skipped` from `inapplicable` from `blocked`; they are kept as the dated record and superseded by this line.

> **Latest measurement:** **2026-06-11 against `@openwop/openwop-conformance@1.22.0` — 1793 passed / 45 failed / 125 skipped / 0 todo of 1963 tests (91.3%)** (default mode; cross-host re-measurement in `INTEROP-MATRIX.md`). *(Re-verified same-day after the fixture-resolution hardening — `OPENWOP_FIXTURES_DIR` override → upward probe → sibling-checkout probe, resolved dir + count logged at boot: identical totals, full 82-fixture catalog, no symlinks.)* The 45 failures are the unchanged honest-non-claim set (advertised-fixture surfaces the minimal host doesn't implement: bulk-cancel, BYOK, interrupts, stream-modes-buffer, dispatch, subworkflow) — **0 failures in any new 1.22.0 file** (this host doesn't advertise webhooks, so `webhook-tenant-isolation` soft-skips). **Measurement-basis note:** this run loads the full 82-fixture catalog (prior readings ran with degraded fixture loading), so passed/skipped deltas vs 1.21.0 reflect broader coverage.
>
> **Prior measurement (superseded 2026-06-11):** **2026-06-09 against `@openwop/openwop-conformance@1.21.0` — 1780 passed / 45 failed / 129 skipped / 0 todo of 1954 tests (91.1%)** (cross-host re-measurement in `INTEROP-MATRIX.md`). The `artifact-auth` 404-vs-401 gap is **closed** (401 now precedes any existence check); the RFC 0073 discovery-root layout was fixed so `nodePackRuntimes`/WASM + other capability families serve at the document root and their scenarios run rather than soft-skip; the 45 remaining failures are honest non-claims for surfaces the minimal host doesn't behaviorally implement (bulk-cancel, BYOK, interrupts, stream-modes-buffer, dispatch, subworkflow, et al.). `it.todo` is 0.
>
...
> **Profile claim (corrected 2026-08-12):** `openwop-core` + `openwop-fixtures` — see §"Committed machine-readable bundle" for the three withdrawn claims and why.
...
- **Profiles claimed (corrected 2026-08-12, RFC 0148 §C/§D):** `openwop-core` + `openwop-fixtures`. **Three claims were withdrawn** — `openwop-stream-sse`, `openwop-stream-poll`, and `openwop-node-packs` — because the bundle's own `results.failed` positively contradicts them: all three `stream-modes*` scenarios failed, as did `pack-registry.test.ts`. Deriving a profile from the discovery document is necessary but **not sufficient**; RFC 0089 §B(2) also requires the floor to be proven, and these did not prove it. Withdrawn rather than the verifier loosened, per RFC 0148 §D. **The two retained claims are discovery-payload-only by design** — their predicates ARE the whole claim, so neither says anything about runtime behavior (RFC 0155 §A). The host **honestly does NOT claim** `openwop-core-standard` because its discovery document omits `clarification.request` from `supportedEnvelopes` (it implements no interrupt path), so `isCoreStandard` is false and the profile is not derivable — exactly the spec's "claim ≠ evidence" point made mechanical.
- **`verifyBundle()` accepts the two retained claims (§B),** re-verified 2026-08-12 against the corrected `PROFILE_FLOOR_SCENARIOS`: both re-derive from the captured `discovery.document` and report `floorProven: true` with `floorUnspecified: false`.
  > **The previous sentence here was the defect, stated as a virtue.** It read: *"none of the claimed profiles defines floor scenarios in `PROFILE_FLOOR_SCENARIOS`, so the floor condition holds vacuously and honestly."* It holds vacuously; it does not hold honestly. `verifyBundleProfile()` computed `floorProven` from `missingFloor.length === 0 && prefixOk` over an **undefined** floor — and `[].every(...)` is `true` — so every claim without a transcribed floor verified as proven having been checked against nothing. RFC 0148 §C transcribed the floors `profiles.md` had stated normatively all along, and three of the five claims immediately failed. An empty floor and an unwritten floor are now distinct values (`discoveryOnly` vs `floorUnspecified`), so "no floor by design" is a decision on record rather than an absence. The bundle's `results.failed` records the host's honest non-claims (interrupts, dispatch, sub-workflows, BYOK, stream-mode buffering, et al.) — none of which touch a *claimed* profile's floor.
