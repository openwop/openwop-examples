...
> **Measurement — 2026-09-05, `@openwop/openwop-conformance@2.0.0-rc.58` + `@openwop/spec-artifacts@2.0.0-rc.58` (npm, corpus stamp VERIFIED), `--target-major 2 --max-workers 4`, **71** scenario files.** Host `openwop-host-v2-reference@2.0.0-rc.1`, build `commit:763e4e3f29b5518cdb8d8862bd23bcbfba3dba3f`, local boot on port 3839, fresh store, one run; the bundle's `discovery.url` names that port and its `signature.keyId` the rotated key — asserted by the cut script before the artifact is kept (see the superseded measurement below for why). Route-level harness 21 / 21.
>
