...
>
> **Every earlier measurement on this page was taken with the egress guard open and said nothing about it in the bundle.** `scripts/cut-bundle.sh` ran this host under `OPENWOP_WEBHOOK_ALLOW_PRIVATE=1` — "required, not optional", because every suite fixture was a loopback listener — and every bundle it produced recorded no relaxation and `certified: true`. `security-defaults.md` §Relaxations requires the relaxation in `host.relaxations[]` and forbids certifying the profile it belongs to. It was written down here, in prose; the rule says the bundle. Disclosed in the corpus (`docs/KNOWN-LIMITS.md`, openwop#1446); found by the steward on the way to flipping RFC 0158 on a loopback cut that certified and was **not** committed.
>
> **This cut is `PUBLIC=1`:** guard CLOSED, and every fixture the host must reach fronted publicly through a tunnel and pinned to a port — the webhook receiver and the RFC 0158 effect receiver, the A2A fake peer, the MCP fake server (the last two only possible since suite 2.33.0), and the synthetic IdP. `scripts/cut-public.sh` opens the four tunnels and verifies they are closed. The suite checks the posture itself: `v2-webhook-egress-refusal` registered eight destinations `webhooks.md` §SSRF says a host MUST refuse — non-`https`, IPv4 and IPv6 loopback, `localhost`, two RFC 1918 ranges, the cloud-metadata address, IPv6 ULA — and all eight answered `400 webhook_url_rejected`. **executed-pass, 2 assertions.** Had one been accepted, `openwop-core-standard` would not have certified.
>
> **What running relaxation-free found that seven relaxed cuts could not** — the undeclared relaxation did not only misstate the bundle; it hid that the guarded path had never once been exercised:
>
> - **This host could not make a request to ANY hostname.** `guardedRequest` pins the connection by passing a custom DNS `lookup`; Node calls it with `all: true` and expects an ARRAY, and this answered in the single-address shape, so every connect to a non-literal URL failed `Invalid IP address: undefined`. An IP-literal URL never reaches `lookup`, so `127.0.0.1` always worked and nothing else ever did — no webhook delivery, no `http.fetch` effect, no IdP, no A2A/MCP peer at a real name, **with the guard open or closed**. Surfaced as `scim provision answered 400`. `test/egress.test.ts` pins it without a network (`localhost` is a name) and is sabotage-proven.
> - **Two suite defects reachable only from this posture**, both fixed upstream: `v2-webhook-delivery-shape` never unregistered, so behind ONE shared public URL a live major-2 subscription delivered into the major-1 leg and failed a host that had rendered both contracts correctly (2.33.1) — reproduced with no ingress by pinning `OPENWOP_WEBHOOK_RECEIVER_PORT` on a loopback run; and the A2A / MCP fakes had no public front at all (2.33.0).
> - **A closed-guard rehearsal with NO ingress** showed exactly which rows need a public front: 10 `blocked` (webhook delivery, the dead-letter read, the A2A/MCP negotiation rows, RFC 0158 `duplicate-delivery`) and 3 `executed-fail` (the SAML/SCIM rows, whose IdP a closed guard cannot reach on loopback). Nothing else moved.
>
> **RFC 0158 — `durable-single-instance`, witnessed.** The host ran under `scripts/supervisor.mjs` with the durability seam mounted; the two kill rows `SIGKILL`ed it and **the supervisor itself counted 2 deaths** (the script refuses a bundle whose kill rows passed with fewer deaths than kill rows). All five rows `executed-pass` with EMPTY detail: `kill-after-accept` (1), `kill-during-execution` (2 — never observable as completed un-re-executed, AND resumed within the declared bound), `duplicate-delivery` (2 — **one arrival at the suite's own receiver**, through the tunnel, for work delivered twice), `bound-is-derived` (1 — a paper check, and labelled one), `poison-exhaustion` (3 — read from `/events/poll`; on 2.31.x it read the SSE operation as JSON and passed vacuously here). **Not claimed and not witnessed:** `durable-multi-instance` (no peer ever resumes anything on a single process), `multi-region-qualified`. The supervisor's restart delay is the OPERATOR's term of the recovery bound and is declared as one.
>
> **`skipped 7`** — the three declared opt-outs (`family.forms`, `family.memory`, `connections.packsSupported`), unchanged. **`inapplicable 6`** and the eight `partial-witness:` notes are all `inapplicable` legs of passing rows (no 429 observed, no windowed revocation lane, the noop fixture records no effect, …); none is `blocked`, none touches RFC 0158.
>
> **Key rotation `v2-reference-3` → `v2-reference-4`.** The `-3` private half was not on the cutting machine — the third rotation for the same reason (gitignored, and lived only in a worktree later removed). From `-4` the private half is also kept outside the repository, so the next cut copies it in instead of rotating. `-3`'s public key stays verifiable from git history, and the 2.25.0 bundle it signed is superseded, not invalidated.

---

> **SUPERSEDED measurement — cut with the egress guard open and no relaxation recorded (see above).**
>
