...
12. **Bundle** — [`bundle-v3.json`](./bundle-v3.json), signed with `keys/host.pem` (Ed25519, key id `v2-reference-1`; public key committed at `keys/host.pub.pem`): witness `ae2590a769c0…`, 183 requirement rows (107 explicit), 1316 assertions, `executedFail: 0`, schema-valid, signature verified, zero verifier rejections, certifying nothing because `blocked > 0`. The §F **Witness** and **Coexistence** cut gates pass outright. See `conformance.md`.
