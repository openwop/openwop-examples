...
| `OPENWOP_MCP_SERVERS` | _(unset)_ | **RFC 0204.** `id=url,id=url` — the MCP servers pack code may address through `ctx.mcp`. `mcp.client` (and the `conformance-mcp-client` fixture, and `mcp` in `toolCatalog.sources`) is advertised only when at least one is bound and the installed `@openwop/spec-artifacts` defines the facet (2.36.0+). A conformance cut binds `conformance` to the suite's fake MCP server (pin `OPENWOP_MCP_FAKE_SERVER_PORT`) and `conformance.down` to an address nothing answers; `scripts/cut-bundle.sh` does both |
...
15. **`ctx.mcp` + the tool catalog (RFC 0204)** — `src/mcp-client.ts` is the MCP 2026-07-28 client pack code gets when `mcp.client` is advertised: `callTool` resolves the server's `CallToolResult` unaltered (`isError: true` is a result, not a rejection; MRTR `input_required` is handled here and never returned), `listTools` one page unaltered with the pack's cursor forwarded, `readResource` the `ReadResourceResult`, `serverHealth` `reachable` / `unreachable` / `incompatible` from a `server/discover` probe reused within its `ttlMs` — never a session state. Rejections: `not_found` (unknown `serverId`), `mcp_error` (the MCP `Error` in `details.error`), `mcp_unreachable`. The revision is decided by the same `decide()` as the §23 seam and audited as `negotiation.decided` on the calling run's log. `core.conformance.mcp-client` (the fixture's node) records the resolved value as `outputs.result`. `src/tool-catalog.ts` serves `GET /tools` / `GET /tools/{toolId}` (`toolCatalog`, `sources: [node-pack, mcp]`): the executor's node types classified by hand, plus every page of every bound server's tools — none of which this host classifies, so every MCP tool is `safetyTier: "write"` whatever its `annotations` claim; `annotations` (2.36.0+ contract) are computed from the descriptor's own `safetyTier` / `replayPolicy` / `egress`; the list is sorted by `toolId`.
...
│   ├── mcp-client.ts  RFC 0204: ctx.mcp — the MCP 2026-07-28 client pack code gets
│   ├── tool-catalog.ts RFC 0204: GET /tools — node types classified by hand + bound MCP servers
